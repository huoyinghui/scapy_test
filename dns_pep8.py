                                            ...
                                            for ar_ttl_ele in ar_ttl_list:
                                                for ar_rclass_ele in ar_rclass_list:
                                                    for ar_rdlen_ele in ar_rdlen_list:
                                                        ...
                                                        dns_query = DNS(
                                                            id=0, qr=qr_ele, opcode=opcode_ele, tc=tc_ele, rd=rd_ele, qdcount=qd_ele, ancount=an_ele, nscount=ns_ele, arcount=ar_ele)
                                                        query_type = random.sample(
                                                            qtype_list, 1)
                                                        dns_query.qd = DNSQR(
                                                            qname=domain_name, qtype=query_type, qclass=qclass_ele)
                                                        dns_query.ar = DNSRR(
                                                            type=ar_type_ele, ttl=ar_ttl_ele, rclass=ar_rclass_ele, rdlen=ar_rdlen_ele)
