...
def check(*index_list):
    for index, x in enumerate(index_list):
        ...
        if x > xmax:
            print("{} index out of range, you input {}, max value is {}".format(key, x, xmax))
            return False
    return True
...
def index_data_table(x0=0, x1=0, x2=0, x3=0, x4=0, x5=0, x6=0, x7=0, x8=0, x9=0, x10=0, x11=0, x12=0):
    ...
    if not check(*index_list):
        print("index err: {}".format(index_list))
        return None
    qr_ele = qr_list[x0]
...
def create_all_dns_index():
    ...
    # c = conn.cursor()
    # 一次创建所有的数据，写入到数据库中, id: 1, 2, ...n
    # 遍历每一条数据，获取坐标，动态生成数据

    for k in xmax_map.keys():
        print(k, xmax_map[k])
    return
...
def do_job():
    # x0,x1,x2,x3,..x12
    cnt = 0
    length = len(xmax_map.keys())
    value = 0
    for k, v in xmax_map:
        cnt += 1
        value += v * (10 ** (length-cnt))
        print(value)
